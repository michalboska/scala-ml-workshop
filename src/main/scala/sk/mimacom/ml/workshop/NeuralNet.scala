...
object NeuralNet extends App {
  ...
  val conf = new SparkConf().setAppName(s"LDAExample").setMaster("local[*]").set("spark.executor.memory", "4g").set("spark.sql.warehouse.dir", "file:///spark-warehouse")
  val spark = SparkSession.builder().config(conf).getOrCreate()
  val sc = spark.sparkContext
  val nn = new NeuralNet(sc, spark)

  nn.run()
}

class NeuralNet(sc: SparkContext, spark: SparkSession) {

  def run(): Unit = {
    println("[Loading data...]")
    val df = loadDataToDataFrame(spark)
    var Array(training, test) = splitTrainingAndTestingData(df)
    val pipeline: Pipeline = getPrepearedNeuralNet
  ...
  private def getPrepearedNeuralNet = {
    ...
    val layersTail = Range(1, 20).map(_ => 15).toArray
    val layers = Array[Int](42) ++ layersTail
    ...
    val pipeline = new Pipeline().setStages(
      Array(jobIndexer, maritalIndexer, educationIndexer, contactIndexer, monthIndexer, poutcomeIndexer,
        jobEncoder, maritalEncoder, educationEncoder, contactEncoder, monthEncoder, poutcomeEncoder, assemblerFeatures, nn)
    )
    pipeline
  }
}
